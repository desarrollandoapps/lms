    ...
    <nav class="mt-2">
      @auth
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
            ...
            <li class="nav-item has-treeview {{ ($leccion->unidad_id == $u->id) ? ' menu-open' : ''}} ">
              <a href="#" class="nav-link {{ ($leccion->unidad_id == $u->id) ? ' active' : '' }}">
                {{-- <i class="nav-icon fas fa-flask"></i> --}}
                <p class="text-wrap pr-4">
                  {{ $u->nombre }}
                  <i class="right fas fa-angle-left"></i>
                </p>
              </a>
              <ul class="nav nav-treeview">
                @foreach ($leccionesAll as $l)
                  @if ($l->unidad_id == $u->id)
                      <li class="nav-item">
                      <a href="{{ route('ver-leccion', $l->id) }}" class="nav-link{{ $id == $l->id ? ' active' : '' }}">
                          {{-- <i class="nav-icon fas fa-flask"></i> --}}
                          <p class="text-wrap pl-2">{{ $l->nombre }}</p>
                      </a>
                      </li>
                  @endif
                @endforeach
